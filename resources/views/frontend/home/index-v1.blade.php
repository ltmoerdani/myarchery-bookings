...
<section class="hero-section overlay pt-105 pb-120 lazy" data-bg="{{ asset('assets/admin/img/hero-section/' . $heroSection->background_image) }}">
  ...
  <section class="events-section pt-110 rpt-90 pb-90 rpb-70 bg-lighter">
    <div class="container">
      ...
      @if (count($eventCategories) < 1) <p class="text-center">{{ __('No Events Found') }}</p>
        @else
        ...
        <div class="row events-active">
          ...
          @php
          $event_type = DB::table('event_type')
            ...
            <div class="event-item">
              <div class="event-image">
                <a href="{{ route('event.details', [$event->slug, $event->id]) }}">
                  <img src="{{ asset('assets/admin/img/event/thumbnail/' . $event->thumbnail) }}" height="300" width="auto" alt="Event">
                </a>
              </div>
              <div class="event-content">
                <ul class="time-info">
                  ...
                  <li>
                    ...
                    <span title="{{ __('Event Duration') }}">{{ $event->date_type == 'multiple' ? @$event_date->duration : $event->duration }}</span>
                  </li>
                  <li>
                ...
                @endphp
                @if ($organizer)
                <a href="{{ route('frontend.organizer.details', [$organizer->id, str_replace(' ', '-', $organizer->username)]) }}" class="organizer">{{ __('By') }}&nbsp;&nbsp;{{ $organizer->username }}</a>
                @endif
                @else
                @php
                $admin = App\Models\Admin::first();
                @endphp
                <a href="{{ route('frontend.organizer.details', [$admin->id, str_replace(' ', '-', $admin->username), 'admin' => 'true']) }}" class="organizer">{{ $admin->username }}</a>
                @endif
                <h5>
                  <a href="{{ route('event.details', [$event->slug, $event->id]) }}">
                ...
                <div class="price-remain">
                  ...
                  <span>
                    ...
                    <span class="price" dir="ltr">
                      ...
                      $variation,
                      function ($a, $b) {
                      return $a['price'] < $b['price'] ? $a : $b; }, array_shift($variation), ); $price=$v_min_price['price']; @endphp <span class="price">
              ...
              @endphp
              @endif
              <a href="{{ $checkWishList == false ? route('addto.wishlist', $event->id) : route('remove.wishlist', $event->id) }}" class="wishlist-btn {{ $checkWishList == true ? 'bg-success' : '' }}">
                <i class="far fa-bookmark"></i>
              </a>
            </div>
          </div>
          @else
          @if (strtolower($event_type->event_type) != 'private')
          <div class="col-lg-4 col-md-6 item {{ $item->id }} motivational">
            <div class="event-item">
              <div class="event-image">
                <a href="{{ route('event.details', [$event->slug, $event->id]) }}">
  ...
  <section class="feature-section pt-110 rpt-90 bg-lighter">
    ...
    <div class="container pb-90 rpb-70">
      <div class="section-title text-center mb-55">
        ...
        @if (count($featureEventItems) < 1) <h2>{{ __('No data found for features section') }}</h2>
          @endif
      </div>
    ...
    <div class="container">
      ...
      <div class="client-logo-wrap">
        ...
        <div class="client-logo-item">
          <a href="{{ $item->url }}" target="_blank"><img class="lazy" data-src="{{ asset('assets/admin/img/partner/' . $item->image) }}" alt="Client Logo"></a>
        </div>
        @endforeach
        @else

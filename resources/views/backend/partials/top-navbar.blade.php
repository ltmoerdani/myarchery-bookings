    ...
    <div class="logo-header" data-background-color="{{ $settings->admin_theme_version == 'light' ? 'white' : 'dark2' }}">
        @if (!empty($websiteInfo->logo))
            <a href="{{ route('index') }}" class="logo" target="_blank">
                <img src="{{ asset('assets/admin/img/' . $websiteInfo->logo) }}" alt="logo" class="navbar-brand"
                    width="120">
            </a>
        @endif
                    ...
                    <div class="form-group">
                        <div class="selectgroup selectgroup-secondary selectgroup-pills">
                            <label class="selectgroup-item">
                                <input type="radio" name="admin_theme_version" value="light"
                                    class="selectgroup-input"
                                    {{ $settings->admin_theme_version == 'light' ? 'checked' : '' }}
                            ...
                            <label class="selectgroup-item">
                                <input type="radio" name="admin_theme_version" value="dark"
                                    class="selectgroup-input"
                                    {{ $settings->admin_theme_version == 'dark' ? 'checked' : '' }}
                                    onchange="this.form.submit()">
                                <span class="selectgroup-button selectgroup-button-icon"><i
                            ...
                            <li>
                                <div class="user-box">
                                        ...
                                        @if (!empty(Auth::guard('admin')->user()))
                                            @if (Auth::guard('admin')->user()->image != null)
                                                <img src="{{ asset('assets/admin/img/admins/' . Auth::guard('admin')->user()->image) }}"
                                                    alt="Admin Image" class="avatar-img rounded-circle">
                                            @else
                                                <img src="{{ asset('assets/admin/img/blank_user.jpg') }}"
                                    ...
                                    <div class="u-text">
                                        ...
                                        <p class="text-muted">
                                            {{ !empty(Auth::guard('admin')->user()) ? Auth::guard('admin')->user()->email : '' }}
                                        </p>
                                    </div>
                                </div>
                            </li>

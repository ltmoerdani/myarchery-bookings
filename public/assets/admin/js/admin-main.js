...
$(function ($) {
    ...
    $.ajax({
      ...
      error: function (error) {
        ...
        for (let x in error.responseJSON.errors) {
          document.getElementById("err_" + x).innerHTML =
  ...
  $('select[name="language_id"]').on("change", function () {
    ...
    let rtlURL =
      baseUrl + "/admin/language-management/" + $(this).val() + "/check-rtl";
    ...
    $.get(rtlURL, function (response) {
      ...
      if ("successData" in response) {
        if (response.successData == 1) {
          ...
          $(
            "form.create .note-editor.note-frame .note-editing-area .note-editable"
          ).each(function () {
            if (!$(this).hasClass("ltr")) {
              $(this).addClass("rtl");
            }
...
$("body").on("change", "#country_id", function () {
  $(".request-loader").addClass("show");
  $.get(
    baseUrl + "/admin/get-state-city/" + $(this).val(),
...
$(".btn-generate-code").on("click", function (e) {
  ...
  $(".btn-generate-code").attr("disabled", "disabled");
  $(".btn-generate-code").html("Loading...");

  $.ajax({
    url: `${base_url}/api/generate-code`,
    method: "GET",
    contentType: false,
    success: function (data) {
      $("#code").val(data);
      $(".btn-generate-code").removeAttr("disabled");
      $(".btn-generate-code").html(
